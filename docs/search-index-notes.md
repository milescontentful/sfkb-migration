...
- 15:11Z: queued a manual **Full Refresh** on `Knowledge_kav_Home` (UI Refresh Now → Full Refresh); it sat PENDING ~10 min (the 15:14 scheduled incremental was skipped), then IN_PROGRESS. Manual refresh is a real lever for the stream hop, but not instant.
- 15:22Z: **search fallback shipped** — `/api/search` now answers from Contentful's own full-text search (`query=`, phrase then per-word stems) whenever the Data 360 index has no Contentful-backed hit; results labeled "keyword match"; Data 360 wins whenever it has results. Verified live on 5 ZoomInfo queries before the index caught up. This is the demo safety net for the publish→searchable lag.
- **15:37Z: index READY with exactly the 20 ZoomInfo articles** (84 chunks, 0 orphans). Timeline: manual Full Refresh queued 15:11 → stream IN_PROGRESS 15:14 → snapshot flipped 15:25 (14 min) → Rebuild submitted 15:26 → IN_PROGRESS 15:32 → READY 15:37 (11 min). Live search switched itself from the Contentful fallback to Data 360 results: "how do credits work" 86%, "authenticate with the api" 87%, "my salesforce sync failed" 88%, "find contacts at a company" 85%.
